...
def conversion(model_dir, model_name, data_dir, conversion_direction, output_dir):
    ...
    for file in os.listdir(data_dir):
        ...
        stft = librosa.core.stft(wav, n_fft=256)
        #f0, timeaxis, sp, ap = world_decompose(wav = wav, fs = sampling_rate, frame_period = frame_period)
        #coded_sp = world_encode_spectral_envelop(sp = sp, fs = sampling_rate, dim = num_features)
        #coded_sp_transposed = coded_sp.T
        
        converted = model.test(inputs=np.array([stft]).astype(np.csingle), direction = conversion_direction)[0]
        ...
        '''
        y = librosa.core.istft(converted)
        librosa.output.write_wav(os.path.join(output_dir, os.path.basename(file)), y, sampling_rate)
