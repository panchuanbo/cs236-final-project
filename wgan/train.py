...
def train(train_A_dir, train_B_dir, model_dir, model_name, random_seed, validation_A_dir, validation_B_dir, output_dir, tensorboard_log_dir):
    ...
    np.savez(os.path.join(model_dir, 'mcep_normalization_A.npz'), *coded_sps_A_norm)
    np.savez(os.path.join(model_dir, 'mcep_normalization_B.npz'), *coded_sps_B_norm)
    '''

    # Load values from numpy saved files.
    mcp_file = np.load(os.path.join(model_dir, 'mcep_normalization.npz'))
    coded_sps_A_mean, coded_sps_A_std, coded_sps_B_mean, coded_sps_B_std = mcp_file['mean_A'], mcp_file['std_A'], mcp_file['mean_B'], mcp_file['std_B']
    coded_sps_A_norm = np.load(os.path.join(model_dir, 'mcep_normalization_A.npz'))
    cur = []
    for i in range(len(os.listdir(train_A_dir))):
        cur.append(coded_sps_A_norm['arr_' + str(i)])
    coded_sps_A_norm = cur
    coded_sps_B_norm = np.load(os.path.join(model_dir, 'mcep_normalization_B.npz'))
    cur = []
    for i in range(len(os.listdir(train_B_dir))):
        cur.append(coded_sps_B_norm['arr_' + str(i)])
    coded_sps_B_norm = cur
    log_file = np.load(os.path.join(model_dir, 'logf0s_normalization.npz'))
    log_f0s_mean_A, log_f0s_std_A, log_f0s_mean_B, log_f0s_std_B = log_file['mean_A'], log_file['std_A'], log_file['mean_B'], log_file['std_B']
    ...
    for epoch in range(num_epochs):
        ...
        for i in range(n_samples // mini_batch_size):
            ...
            for k in range(n_critic):
                discriminator_loss = model.train_critic(input_A = dataset_A[start:end], input_B = dataset_B[start:end], lambda_cycle = lambda_cycle, lambda_identity = lambda_identity, discriminator_learning_rate = discriminator_learning_rate)

            generator_loss = model.train_generator(input_A = dataset_A[start:end], input_B = dataset_B[start:end], lambda_cycle = lambda_cycle, lambda_identity = lambda_identity, generator_learning_rate = generator_learning_rate)
