...
class CycleGAN(object):
    ...
    def build_model(self):
        ...
        # In Wasserstein, generator wants to maximize the critic score.
        self.generator_loss_A2B = -tf.reduce_mean(self.discrimination_B_fake)
        self.generator_loss_B2A = -tf.reduce_mean(self.discrimination_A_fake)
        ...
        self.discriminator_loss_A = tf.reduce_mean(self.discrimination_input_A_real) - tf.reduce_mean(self.discrimination_A_fake)
        ...
        self.discriminator_loss_B = tf.reduce_mean(self.discrimination_input_B_real) - tf.reduce_mean(self.discrimination_B_fake)
    ...
    def optimizer_initializer(self):
        ...
        self.discriminator_optimizer = tf.train.RMSPropOptimizer(learning_rate = self.discriminator_learning_rate).minimize(self.discriminator_loss, var_list = self.discriminator_vars)
        ...
        self.generator_optimizer = tf.train.RMSPropOptimizer(learning_rate = self.generator_learning_rate).minimize(self.generator_loss, var_list = self.generator_vars)

    def train_generator(self, input_A, input_B, lambda_cycle, lambda_identity, generator_learning_rate):
    ...
    def train_critic(self, input_A, input_B, lambda_cycle, lambda_identity, discriminator_learning_rate):
        ...
        discriminator_loss, _, discriminator_summaries = self.sess.run([self.discriminator_loss, self.discriminator_optimizer, self.discriminator_summaries], \
            feed_dict = {self.input_A_real: input_A, self.input_B_real: input_B, self.discriminator_learning_rate: discriminator_learning_rate})
        self.discrimination_A_fake = tf.clip_by_value(self.discrimination_A_fake, -.01, .01) # Clipping weights of critic
        self.discrimination_B_fake = tf.clip_by_value(self.discrimination_B_fake, -.01, .01)
